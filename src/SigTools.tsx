...
export const SignatureTools = () => {
    ...
    const generateKey = () => {
        if (signAlgorithm === 'ECDSA') {
            const keys = secp256k1KeyGenerator();

            setPrivateKey(keys.privateKey);
            setPublicKey(keys.publicKey);
            setUncompressedPublicKey(keys.uncompressedPubKey);
        } else {
            const keys = schnorrKeyGenerator();
    ...
    const messageSign = () => {
        ...
        if (signAlgorithm === 'ECDSA') {
            try {
                const messageHash = WizData.fromHex(sha256(WizData.fromHex(signMessage)).toString());
                const signResult = secp256k1Sign(messageHash, privateKey);
            ...
            try {
                const signResult = schnorrSign(WizData.fromHex(signMessage), privateKey);
    ...
    const createPublicKey = () => {
        if (signAlgorithm === 'ECDSA') {
            try {
                const keys = secp256k1CreatePublicKey(WizData.fromHex(privateKeyInput));
            ...
            try {
                const keys = schnorrCreatePublicKey(WizData.fromHex(privateKeyInput));
    ...
    const messageVerify = () => {
        ...
        if (signAlgorithm === 'ECDSA') {
            try {
                const verifyResult = ecdsaVerify(wizDataVerifySignature, wizDataVerifyMessage, wizDataVerifyPublicKey);
            ...
            try {
                const verifyResult = shnorrSigVerify(wizDataVerifySignature, wizDataVerifyMessage, wizDataVerifyPublicKey);
                setVerifyResultNumber(verifyResult.number);
            } catch (err) {
                setVerifyErrorMessage(err as string);
            }
